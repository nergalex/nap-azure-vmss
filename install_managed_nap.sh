...
# Controller - get auth token for API access #
    echo 'Controller - get auth token'
...
# Controller - get license keys #
    echo 'Controller - get license keys'
...
# INSTALL NGINX Javascript module needed for APIM #
    echo 'INSTALL NGINX Javascript module needed for APIM'
    yum update && yum -y install "nginx-plus-${EXTRA_NGINX_PLUS_VERSION}*" "nginx-plus-module-njs-${EXTRA_NGINX_PLUS_VERSION}*"
# INSTALL NGINX App Protect #
...
# Controller - SET agent specification #
    echo 'Controller - set agent specification'
...
# Controller - RUN agent #
    echo 'Controller - run agent, configure instance then start nginx'
    bash ./install.sh -y
